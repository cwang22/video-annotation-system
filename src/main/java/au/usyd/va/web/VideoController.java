...
public class VideoController {
	...
	public String addAnnotation(HttpServletRequest httpServletRequest) {
		...
		Video video = this.videoManager.getVideoById(id);
		JsonArray array = parser.parse(jsonString).getAsJsonObject()
				.getAsJsonArray("time");
		System.out.println(array);
		for (int i = 0; i < array.size(); i++) {
			VideoAnnotation va = gson.fromJson(array.get(i),
	...
	public String selectKeyFrame(@PathVariable("id") Long id,
			HttpServletRequest httpServletRequest, Model uiModel) {
		Video video = this.videoManager.getVideoById(id);
		List<VideoAnnotation> vas = this.videoAnnotationManager
				.getAnnotations(video);
